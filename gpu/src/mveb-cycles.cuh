...
void permutevEB_balanced(TYPE *dev_A, uint64_t n, uint64_t m, uint32_t d) {
    #ifdef DEBUG
    printf("\npermutevEB_balanced: n = %lu; m = %lu; d = %u\n", n, m, d);
    #endif
...
void permutevEB_nonperfect(TYPE *dev_A, uint64_t n, uint32_t d) {
    ...
    equidistant_gather_phaseOne<TYPE><<<blocks, threads>>>(dev_A, num_full, l);
    #ifdef DEBUG
    cudaError_t cudaerr = cudaDeviceSynchronize();
    if (cudaerr != cudaSuccess) {
        printf("equidistant_gather_phaseOne failed with error %i \"%s\".\n", cudaerr, cudaGetErrorString(cudaerr));
    ...
    equidistant_gather_phaseTwo<TYPE><<<blocks, threads>>>(dev_A, num_full, l);
    #ifdef DEBUG
    cudaerr = cudaDeviceSynchronize();
    if (cudaerr != cudaSuccess) {
        printf("equidistant_gather_phaseTwo failed with error %i \"%s\".\n", cudaerr, cudaGetErrorString(cudaerr));
