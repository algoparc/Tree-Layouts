...
#ifndef BST_HYBRID_CUH
#define BST_HYBRID_CUH
...
void permute_leaves(TYPE *dev_A, uint64_t n, uint64_t numInternals, uint64_t numLeaves) {
    extended_equidistant_gather2<TYPE>(dev_A, 2*numLeaves, 1);

    shift_right_phaseOne<TYPE><<<BLOCKS, THREADS>>>(&dev_A[numLeaves], numInternals, numInternals - numLeaves);
    #ifdef DEBUG
    ...
    shift_right_phaseTwo<TYPE><<<BLOCKS, THREADS>>>(&dev_A[numLeaves], numInternals, numInternals - numLeaves);
    #ifdef DEBUG
